...
BATCH_SIZE = 512
MAX_EPOCHS = 300
PATIENCE   = 15 # early stopping patienc
...
class CNN1DRegressor(nn.Module):
    def __init__(self, cin, out_dim=2,
                chs = (130,110,90,70,50,30,20),
                ks = (55, 20, 30, 15, 17, 10, 5),
                p_drop=0.1):
        super().__init__()
        ...
        for i, (c_out, k) in enumerate(zip(chs, ks)):
            ...
            # Pool at layers 0, 2, 4 (more gradual)
            if i in [0, 2, 4]:
...
log_path = os.path.join("results", f"1_training_log{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
...
}).to_csv(f"results/1_test_predictions{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", index=False)
print("Saved predictions to results/1_test_predictions.csv")
...
png_path = os.path.join("figs", f"1_pred_vs_true_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
