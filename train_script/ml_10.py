...
# ---------------- Sampler: balance joint (RHI, fcnm) bins in TRAIN ----------------
# labels came from stratify_labels(RHI, fcnm, ...)
train_bins = labels[train_idx]          # use the same 2D bins as for stratified split

u, c = np.unique(train_bins, return_counts=True)
count_map = {b: n for b, n in zip(u, c)}

# Inverse-frequency weights: rare (RHI,fcnm) regions get sampled more often
weights = np.array([1.0 / count_map[b] for b in train_bins], dtype=np.float64)
...
log_path = os.path.join("results", f"10_training_log{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
...
}).to_csv(f"results/10_test_predictions{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", index=False)
print("Saved predictions to results/10_test_predictions.csv")
...
png_path = os.path.join("figs", f"10_pred_vs_true_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
