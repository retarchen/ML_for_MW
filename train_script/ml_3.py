...
log_path = os.path.join("results", f"3_training_log{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
...
}).to_csv(f"results/3_test_predictions{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", index=False)
print("Saved predictions to results/3_test_predictions.csv")
...
png_path = os.path.join("figs", f"3_pred_vs_true_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
